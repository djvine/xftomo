...
#constants 
base_direc = '/home/david/data/Gleber'
images_direc = base_direc+'/images'
align_direc = base_direc+'/align'
sinograms_direc = base_direc+'/sinograms'
tomograms_direc = base_direc+'/tomograms'
raw_data_direc = base_direc+'/h5'
animations_direc = base_direc+'/animations'
		...
		for (fnumber, theta) in theta_filenumber:
			...
			if math.isnan(theta): 
				...
				try:
					...
					sp.misc.toimage(data).save(images_direc+'/raw_channel_{0:d}_angle_{1:3.2f}_mda_{2:04d}.png'.format(int(channel), theta, int(fnumber)))
...
class tomo_recon(tomo_data):
	...
	def self_absorption_correction(self):
		"""

		Self-absorption correction is applied iteratively to each projection.

		Use ART. Take log of I and factor in yield and depth from entrance surface.
		Normalise to measured projection.
...
def standard_tomo_reconstruction():
	...
	"""
	path = '/home/david/data/Gleber/h5'
	mda2theta_fn = '/home/david/data/Gleber/Gleber-Comm1-MDA_Angles.tsv'
	ref_channel = 1
	hdfpath_xrf = 'MAPS/XRF_roi'
	hdfpath_dpc = 'MAPS/scalers'
	...
	data = tomo_recon(path, 
					 ...
					 hdfpath=hdfpath_xrf, 
					 hdf_channel_names=hdf_channel_names
					)
	data.align_data(align_to = '13.5')
	...
	l.pop(ref_channel)
	l=[6, 7, 8, 10, 11, 12, 15, 18, 19, 20, 21]
	for i in l:
		data.read_multi_files(path, channel=i, hdfpath=hdfpath_xrf, hdf_channel_names=hdf_channel_names)
		data.align_data()
		data.create_sinogram()
		data.reconstruct_sinogram()
	
	# Reconstruct DPC channels
	if False:
		l=range(5,11)
		for i in l:
			data.read_multi_files(path, channel=i, hdfpath=hdfpath_dpc, hdf_channel_names='hdf_channel_names')
			data.align_data()
			data.create_sinogram()
			data.reconstruct_sinogram()
